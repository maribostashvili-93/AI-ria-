...
import { COMPONENT_LIBRARY, ComponentSpec, ProjectTemplate, detectTemplate } from "./templates.js";
...
export interface UiPlan {
  ...
  agents: { name: string; role: string; pack: string; packBudget: number; tokenLimit: number; order: number }[];
  securityFlows: string[];
}

/** Read whatever project description exists (README, package.json description). */
...
function pickAgents(template: ProjectTemplate): UiPlan["agents"] {
  const names: { name: string; role: string; pack: string }[] = [
...
function templateOf(plan: UiPlan): ProjectTemplate {
...
export function visualPackFromPlan(plan: UiPlan): string {
  ...
  for (const c of plan.components) {
    ...
    if (c.security) lines.push(`- ⚠ ${c.security}`);
    lines.push("");
  }
...
export async function writeUiPlan(root: string, goal: string): Promise<PlanUiResult> {
  ...
  // Design pack combines design memory + the new DESIGN.md
  ...
  // Token accounting: the visual pack replaces reading the whole plan + design docs raw.
